...
class ScenarioConfigurationParser(object):
    ...
    def parse_scenario_configuration(scenario_config_file, scenario_name):
        ...
        for scenario in tree.iter("scenario"):
            ...
            for weather in scenario.iter("weather"):
                ...
                new_config.weather.wind_intensity = float(weather.attrib.get("fog_density", 0.35))
                new_config.weather.sun_azimuth = float(weather.attrib.get("fog_distance", 0.0))
                new_config.weather.sun_altitude = float(weather.attrib.get("wetness", 15.0))
            ...
            for pedestrian in scenario.iter("pedestrian"):
                new_config.start_distance = float(pedestrian.attrib.get("start_distance", 0))
                new_config.walker_target_velocity = float(pedestrian.attrib.get("target_velocity", 0))
                new_config.walker_yaw = float(pedestrian.attrib.get("yaw", 0))

            for mu in scenario.iter("friction"):
                new_config.friction = float(mu.attrib.get("mu", 0))
            ...
            for other_actor in scenario.iter("other_actors"):
    ...
    def get_list_of_scenarios(config_file_name):
        ...
        list_of_config_files = glob.glob(
            "C:\CARLA\scenario_runner-0.9.8\srunner\examples\*.xml")
        list_of_config_files += glob.glob(
            "C:\CARLA\scenario_runner-0.9.8\srunner\examples\*.xosc")
    ...
    def find_scenario_config(scenario_name, config_file_name):
        ...
        #list_of_config_files = glob.glob("{}/srunner/examples/*.xml".format(os.getenv('ROOT_SCENARIO_RUNNER', "./")))
        list_of_config_files = glob.glob("C:\CARLA\scenario_runner-0.9.8\srunner\examples\*.xml")
