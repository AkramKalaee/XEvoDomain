...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False,
                 debug_mode=False, criteria_enable=True, adversary_type=False, timeout=30):
        """
        Setup all relevant parameters and create scenario
        """
        self._wmap = CarlaDataProvider.get_map()
        ...
        self._reference_waypoint = self._wmap.get_waypoint(config.trigger_points[0].location)
        # ego vehicle parameters
        self._ego_vehicle_distance_driven = config.start_distance
        # other vehicle parameters
        self._other_actor_target_velocity = config.walker_target_velocity
        self._other_actor_max_brake = 1.0
        self._time_to_reach = 10
        self._adversary_type = adversary_type  # flag to select either pedestrian (False) or cyclist (True)
        self._walker_yaw = config.walker_yaw
    ...
    def _calculate_base_transform(self, _start_distance, waypoint):
        ...
        waypoint = self._wmap.get_waypoint(location)
        #param3
        offset = {"orientation": self._walker_yaw, "position": 90, "z": 0.6, "k": 1.0}
        position_yaw = waypoint.transform.rotation.yaw + offset['position']
        orientation_yaw = waypoint.transform.rotation.yaw + offset['orientation']
        offset_location = carla.Location(
            offset['k'] * lane_width * math.cos(math.radians(position_yaw)),
    ...
    def _spawn_adversary(self, transform, orientation_yaw):
        ...
        if self._adversary_type is False:
            ...
            walker = CarlaActorPool.request_new_actor('walker.pedestrian.0007', transform)
            adversary = walker
        else:
    ...
    def _spawn_friction(self):
        world = self.ego_vehicles[0].get_world()
        friction_bp = world.get_blueprint_library().find('static.trigger.friction')
        extent = carla.Location(2000.0, 1000.0, 0.0)
        ...
        transform = self._reference_waypoint.transform
        # transform.location = carla.Location(-10000.0, -10000.0, 0.0)
        #transform.location.x += -10
        world.spawn_actor(friction_bp, transform)

        world.debug.draw_box(box=carla.BoundingBox(transform.location, extent * 1e-2), rotation=transform.rotation,
                             life_time=100, thickness=0.5, color=carla.Color(r=255, g=0, b=0))

        # bounding_box = friction_bp.bounding_box
    ...
    def _initialize_actors(self, config):
        ...
        # fric_disp_transform = carla.Transform(
        #     carla.Location(self.transform3.location.x,
        #                    self.transform3.location.y,
        #                    self.transform3.location.z - 500),
        #     self.transform2.rotation)

        first_vehicle.set_transform(disp_transform)
        blocker.set_transform(prop_disp_transform)
        #road_friction.set_transform(fric_disp_transform)

        first_vehicle.set_simulate_physics(enabled=False)
        blocker.set_simulate_physics(enabled=False)
        #road_friction.set_simulate_physics(enabled=False)

        self.other_actors.append(first_vehicle)
        self.other_actors.append(blocker)
        #self.other_actors.append(road_friction)
    ...
    def _create_behavior(self):
        ...
        dist_to_trigger = 5 + self._num_lane_changes
        # leaf nodes
        ...
        # scenario_sequence.add_child(ActorTransformSetter(self.other_actors[2], self.transform3,
        #                                                  name='TransformSetterTS3coca', physics=False))
...
#                                                          spawn_point_wp.transform.location.z + 0.3))
#
#         spawn_point_vending = self.ego_vehicles[0].get_world().get_map().get_waypoint(self.transform2.location)
#
#
#         # if spawn_point_vending.lane_type == carla.LaneType.Driving:
#         #     print("wrong location for vending!")
#
#         static = CarlaActorPool.request_new_actor('static.prop.vendingmachine', self.transform2)
#         static.set_simulate_physics(enabled=False)
#
...
#             try:
#                 self.transform, orientation_yaw = self._calculate_base_transform(_start_distance, waypoint)
#                 wp = self._wmap.get_waypoint(self.transform.location)
#                 print("{}".format(wp.lane_type))
...
#                                                        self._time_to_reach)
#
#         init_behavior_ego = SetInitSpeed(self.ego_vehicles[0], init_speed=10, name = "ego init speed")
#
#         init_behavior_other = SetInitSpeed(self.other_actors[0], init_speed=1, name="walker init speed")
#
#         actor_velocity = KeepVelocity(self.other_actors[0],
#                                       self._other_actor_target_velocity,
#                                       name="walker velocity")
...
#         scenario_sequence.add_child(HandBrakeVehicle(self.other_actors[0], True))
#         scenario_sequence.add_child(start_condition)
#         scenario_sequence.add_child(init_behavior_ego)
#         scenario_sequence.add_child(init_behavior_other)
#         scenario_sequence.add_child(HandBrakeVehicle(self.other_actors[0], False))
#         scenario_sequence.add_child(keep_velocity)
#         scenario_sequence.add_child(keep_velocity_other)
#         scenario_sequence.add_child(actor_stop_crossed_lane)
