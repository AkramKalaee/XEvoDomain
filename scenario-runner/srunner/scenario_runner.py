...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        self.client = args.client #carla.Client(args.host, int(args.port), worker_threads=1)
    ...
    def _signal_handler(self, signum, frame):
        ...
        if self.manager:
            ...
            if not self.manager.get_running_status():
                print("akrammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm")
    ...
    def _prepare_ego_vehicles(self, ego_vehicles):
        ...
        if not self._args.waitForEgo:
            ...
            command2 = "python C:/CARLA/carla_0.9.9/PythonAPI/automatic_control.py --port {} -v --scenario {}  --round {} --pathId {} --base_path {} --filter model3 -a Behavior -b normal --res 500x500".format(
                     self._args.port, self._args.scenario, self._args.round, self._args.pathId, self._args.base_path)
            ego_process = subprocess.Popen(command2, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            #stdout, stderr = ego_process.communicate()

            def out_err_thread(out_iter, proc):
                stdout, stderr = proc.communicate()
                print(stdout.decode("utf-8"))
                print(stderr.decode("utf-8"))
            out_iter = []
            _t = threading.Thread(target=out_err_thread, args=(out_iter, ego_process))
            _t.start()
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        if self._args.reloadWorld:
            ...
            settings.fixed_delta_seconds =  1.0 / self.frame_rate  # --> None means real time simulation
    ...
    def _load_and_run_scenario(self, config):
        ...
        try:
            ...
            if config.friction is not None:
                ...
                self.world.debug.draw_box(box=carla.BoundingBox(transform.location, extent * 1e-2),
                                          rotation=transform.rotation,
                                          life_time=100, thickness=0.5, color=carla.Color(r=255, g=0, b=0))
            ...
            _t = self._prepare_ego_vehicles(config.ego_vehicles)
        ...
        try:
            ...
            if self._args.record:
                self.client.start_recorder(
                    "{}\{}.log".format(config.base_path, config.name))
    ...
    def _run_challenge(self):
        ...
        if self._args.challenge:
            ...
            scenario_runner_root = self.client.start_recorder("C:\CARLA\scenario_runner-0.9.8")
...
def srunner_start(srunner_args, client, world, base_path):
        ...
        parser.add_argument('--timeout', default="50.0",
                            help='Set the CARLA client timeout value in seconds')
        # Akram Kalaee
        parser.add_argument('--pathId', default="1",
                            help='Set the pathId for test scenario generation')
        parser.add_argument('--base_path', default="C:/CARLA/experiment",
                            help='Set the pathId for test scenario generation')
        parser.add_argument('--round', default="1",
                            help='Set the current round for test scenario generation')
        parser.add_argument('--client', type=object,
                            help='Set the pathId for test scenario generation')
        parser.add_argument('--world',type=object,
                            help='Set the pathId for test scenario generation')
        parser.add_argument('-v', '--version', action='version', version='%(prog)s ' + str(VERSION))
        arguments = parser.parse_args(srunner_args)
        ...
        try:
            dir_name = base_path
            dir_name = '{}/path_{}'.format(dir_name, arguments.pathId)

            dir_path = '{}/round_{}'.format(dir_name, arguments.round)
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)

            arguments.outputDir = dir_path

            with open('{}/{}.csv'.format(dir_path, arguments.scenario), mode='w', encoding='utf8') as f:
                f.write("")


            dir_name = '{}/{}_coverage'.format(dir_path, arguments.scenario)
            if not os.path.exists(dir_name):
                os.makedirs(dir_name)

            dir_name = '{}/{}_trace'.format(dir_path, arguments.scenario)
            if not os.path.exists(dir_name):
                os.makedirs(dir_name)
