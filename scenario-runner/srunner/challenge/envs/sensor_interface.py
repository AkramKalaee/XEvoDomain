...
class CANBusSensor(object):
    ...
    @staticmethod
    def _on_can_bus_event(weak_self, event):
        self = weak_self()
        if not self:
            return
        self._frame = self.__call__()
        self._frame = event.frame

    def prepare_CAN_Bus(self):
        timestamp = self._world.wait_for_tick(seconds=2.0).timestamp
        self._frame = timestamp.frame_count
        self._callback(CANBusMeasurement(self.__call__(), self._frame))
